...
(function($) {
  ...
  THEMEMASCOT.isLTR = {
	...
	if ($('.testimonial-carousel-seven').length) {
		$('.testimonial-carousel-seven').owlCarousel({
			rtl: THEMEMASCOT.isRTL.check(),
			loop: true,
			margin: 0,
			nav: false,
	...
	// Testimonial Carousel
	if ($('.testimonial-carousel-eight').length) {
		$('.testimonial-carousel-eight').owlCarousel({
			rtl: THEMEMASCOT.isRTL.check(),
			loop: true,
			margin: 30,
			nav: false,
	...
	if ($('.project-slider-two').length) {
		var swiper = new Swiper(".project-slider-two", {
			slidesPerView: 1,
			spaceBetween: 30,
			breakpoints: {
				320: {
